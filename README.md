...
If you want to use existing tier list images, there's an easy way to do so using an extension I made, [Media Grabber](https://github.com/milk3e/Media-Grabber-Extension). Simply go to tier list website (or any suitable website with images), click Media Grabber, change the shared folder path to the one that holds specifically tier list image items, then download it as a zip. Then you can just extract the zip holding the images, and dump those images into this tier list maker.
